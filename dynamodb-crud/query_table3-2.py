...
if __name__ == '__main__':
	...
	response = table.query(
    	KeyConditionExpression=Key('transition_airport').eq(airport2),
		KeyConditionExpression=Key('depart_date').eq(int(depart_date))
	)

	print 'Route {}->{}->{} on {}:'.format(airport1, airport2, airport3, depart_date)
	for i in response['Items']:
            if i['origin_airport'] == airport1 and i['dest_airport'] == airport3:
            	print '{}->{} (by {}) then {}->{} (by {}), historical delay: {}'.format(
					airport1, airport2, i['origin_carrier'],
					airport2, airport3, i['dest_carrier'],
					i['delay'])
